...
ggplot(point.scores, aes(x= Meter_from_ditch, y=NMDS1)) +
  geom_point(shape=1, color= "black", size=2.2) +  geom_smooth(method='lm', formula= y~x+sqrt(x), col="black", fill="#008837", lwd=1.2) +
  geom_smooth(method='lm', formula= y~x+sqrt(x), se=FALSE, col="black", lwd=1.2) +
  geom_hline(aes(yintercept = 0.5717491), linetype = "dotted", col = "#810f7c") +
  labs(x = "Avstand fra grøft (m)", y= "NMDS1 skår") +
  theme_bw() +
  annotate(geom = "rect", xmin= 0, xmax= 50, ymin= 0.283538, ymax=0.8599601, fill="#810f7c", alpha=0.3) +
...
myfill <- c("#7b3294","#008837","#c2a5cf","#a6dba0","#e7d4e8","#d9f0d3","#e08214")
