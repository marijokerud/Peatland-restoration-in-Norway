...
ggplot(data=all.point.scores, aes(x=NMDS1, y=NMDS2)) + 
  geom_point(data=all.point.scores, aes(x=NMDS1, y=NMDS2, fill = as.factor(Meter_from_ditch2), color = as.factor(Meter_from_ditch2), shape= AAR2),  size = 4) + # 
  ...
  scale_color_manual(values=myfill,
                     breaks=c("0", "2.5", "3.5", "6.5", "10", "12.5", "13.5", "16.5", "20", "22.5", "23.5", "26.5", "32.5", "33.5", "43.5", "K5")) +
  scale_fill_manual(values=myfill) +
  scale_shape_manual(values = c(21,22,24), labels=c("Before","Two years after", "Seven years after"),name="Restoration") +
  guides(fill = FALSE) +
  theme_bw() +
  theme(axis.title.x = element_text(size=14,hjust=0.5),
        axis.title.y = element_text(size=14,vjust=1),
